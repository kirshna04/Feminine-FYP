...
import { useUpdateSellerProfileMutation } from '../../../services/api\'s/common/userAccountApi';
import ImagePopup from '../../Customer/Profile/ImagePopup';
...
const UpdatePopup = ({ setshowUpdatePopup, data, accountId }) => {
    ...
    const SubmitData = async () => {
        ...
        let result = await updateSellerProfile({ accountId: accountId, profileData: formData })
        if (result?.data) {
            ...
            />
            {
                showImagePopup && (
                    <div className=' absolute top-0 left-0 w-screen h-screen bg-black bg-opacity-50'>
                        <div className='flex justify-center items-center w-[100%] md:w-[80%] h-[100%]'>
