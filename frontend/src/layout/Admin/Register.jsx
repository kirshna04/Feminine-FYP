...
const Register = () => {
    ...
    const submitRegisterationForm = async (e) => {
        ...
        if (!registerInfo?.email || !registerInfo.password || !registerInfo.name) {
            ...
            try {
                let result = await axios.post(`http://localhost:4000/api/v1/admin-registration`,registerInfo)
                if (result?.data) {
        ...
        <>
            <div className='font-popins relative h-screen'>
                ...
                <div className='w-screen h-screen lg:h-[90vh] block sm:flex justify-center items-center'>
                    ...
                    <form onSubmit={(e) => submitRegisterationForm(e)} className='w-screen ml-2 mr-2 block sm:flex justify-between md:justify-around items-center p-2'>

                        {/* LEFT SIDE CONTAINER  */}
                        <div>
                            <h1 className='text-3xl font-bold mb-1 sm:mb-2 mt-[2rem] sm:mt-0'> Register in to</h1>
                            <h1 className='text-2xl mb-2 sm:mb-7'>Feminine Care</h1>
                            <p className='text-base'>If you Already have an account registered</p>
                            <p className='text-base mb-10 sm:mb-0'>You can <span className={`text-[${accountScreensColor.textColor}] cursor-pointer`} onClick={() => nav("/admin/login")}>Login here !</span></p>
                        </div>

                        {/* MAIN INPUT FIELDS  */}
                        <div className='flex ml-0 mr-0 h-[60vh] justify-center items-center flex-col sm:block'>

                            <h1 className='text-2xl mb-4 md:mb-10 hidden sm:block'>Sign up</h1>

                            <Input placeholder={"Enter Your Name"} onChangeFunc={(e) => { onchangeRegisterInput(e) }} type={"text"} name={"name"} className={"mb-6 placeholder:text-[#A7A3FF] bg-[#F0EFFF] w-[20rem] sm:w-[14rem] md:w-[18rem] h-[3rem] rounded-[8px] pl-3 pr-3 outline-none"} />


                            <Input placeholder={"Enter Email Address"} onChangeFunc={(e) => onchangeRegisterInput(e)} type={"email"} name={"email"} className={"mb-6 placeholder:text-[#A7A3FF] bg-[#F0EFFF] w-[20rem] sm:w-[14rem]  md:w-[18rem] h-[3rem] rounded-[8px] pl-3 pr-3 outline-none"} />


                            <div className='relative'>
                                <Input onChangeFunc={(e) => { onchangeRegisterInput(e) }} placeholder={"Enter Your Password"} type={showPassword ? "text" : "password"} name={"password"} className={"mb-6 placeholder:text-[#A7A3FF] bg-[#F0EFFF] w-[20rem] sm:w-[14rem]  md:w-[18rem] h-[3rem] rounded-[8px] pl-3 pr-[3.4rem] outline-none"} />
...

                            {
                                loading ?
                                    ...
                                    <Button text={"Register"} className={`w-[20rem] sm:w-[14rem]  md:w-[18rem] h-[3rem] bg-[${accountScreensColor.buttonBg}] shadow-btnshadow1 text-white rounded-[8px]`} />
                            }
